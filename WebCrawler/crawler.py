...
class Fin:
    ...
    def __init__(self):
        ...
        self.pool = Postgres.get("select distinct stock_id from stock_info where length(stock_id) = 4 and "
                                 "industry_category <> 'ETF' order by stock_id asc;")
    ...
    def taiwanstockper(self):
        ...
        for i in self.pool:
            ...
            Postgres.run("delete from stock_per_pbr where stock_id = '{}' and date > '{}';".format(sid, self.date))
    ...
    def stocksecurities(self):
        ...
        for i in self.pool:
            sid = i[0]
            logger.info('get {} stock dividend'.format(sid))
            Postgres.run("delete from stock_securities where stock_id = '{}' and date >= '{}';".format(sid, self.date))
    ...
    def stockmonthrevenue(self):
        ...
        for i in self.pool:
            ...
            Postgres.run("delete from stock_mom_revenue where stock_id = '{}' and date >= '{}';".format(sid, self.date))
